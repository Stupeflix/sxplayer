...
static const uint8_t *sxpi_avc_find_startcode_internal(const uint8_t *p, const uint8_t *end)
{
...
const uint8_t *sxpi_avc_find_startcode(const uint8_t *p, const uint8_t *end){
    const uint8_t *out= sxpi_avc_find_startcode_internal(p, end);
    if(p<out && out<end && !out[-1]) out--;
    return out;
}

int sxpi_avc_parse_nal_units(AVIOContext *pb, const uint8_t *buf_in, int size)
{
    ...
    size = 0;
    nal_start = sxpi_avc_find_startcode(p, end);
    for (;;) {
        while (nal_start < end && !*(nal_start++));
        if (nal_start == end)
            break;

        nal_end = sxpi_avc_find_startcode(nal_start, end);
...
int sxpi_avc_parse_nal_units_buf(const uint8_t *buf_in, uint8_t **buf, int *size)
{
    AVIOContext *pb;
    ...
    sxpi_avc_parse_nal_units(pb, buf_in, *size);
...
int sxpi_isom_write_avcc(AVIOContext *pb, const uint8_t *data, int len)
{
    ...
    ret = sxpi_avc_parse_nal_units_buf(data, &buf, &len);
    if (ret < 0)
        return ret;
    start = buf;
    end = buf + len;
...
int sxpi_avc_write_annexb_extradata(const uint8_t *in, uint8_t **buf, int *size)
{
...
const uint8_t *sxpi_avc_mp4_find_startcode(const uint8_t *start,
                                           const uint8_t *end,
                                           int nal_length_size)
{
    unsigned int res = 0;
